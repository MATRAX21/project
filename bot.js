const Discord = require('discord.js');  //Xeon#0028
const client = new Discord.Client();  //Xeon#0028
const moment = require('moment');
const zalgo = require('zalgolize');  //Xeon#0028
const math = require('math-expression-evaluator');   //Xeon#0028 
const figlet = require('figlet');   //Xeon#0028
const fs = require('fs');  //Xeon#0028
const ms = require('ms');  //Xeon#0028
const prefix = '-'

client.on('message', msg => {  //Xeon#0028
    if (msg.content === 'باك') {  //Xeon#0028
      msg.reply('** :wink: وِلِـكُمِـ ﻧَوِرُتْ   :sparkling_heart:**');  //Xeon#0028
    }
  });

client.on('message', msg => {  //Xeon#0028
    if (msg.content === 'ويسو') {  //Xeon#0028
      msg.reply('**ويسو مات**');  //Xeon#0028
    }
  });


client.on('message', msg => {  //Xeon#0028
    if (msg.content === 'احبك') {  //Xeon#0028
      msg.reply('**لاتحتك يا اخوي لو سمحت**');  //Xeon#0028
    }
  });
  ...
  client.on('message', msg => {  //Xeon#0028
    if (msg.content === 'احبك') {  //Xeon#0028
      msg.reply('**حبك برص يا سافل**');  //Xeon#0028
    }
  });
   ...
   client.on('message', msg => {  //Xeon#0028
    if (msg.content === 'هلا') {  //Xeon#0028
      msg.reply('**هلا بيك :heart: **');  //Xeon#0028
    }
  });
  ...
  client.on("message", message => {  //Xeon#0028
    if(message.content.startsWith(prefix + 'v2min')) {  //Xeon#0028
     let args = message.content.split(" ").slice(1);  //Xeon#0028
       var nam = args.join(' ');
    
      if(!message.member.hasPermission('ADMINISTRATOR')) return    message.channel.send('`ADMINISTRATOR` للأسف هذه الخاصية تحتاج الى ').then(msg => msg.delete(6000))
      if (!nam) return message.channel.send(`<@${message.author.id}> يجب عليك ادخال اسم`).then(msg => msg.delete(10000))  //Xeon#0028
      message.guild.createChannel(nam, 'voice').then(c => setTimeout(() => c.delete(), 120000))   //Xeon#0028
...
client.on("message", message => {    //Xeon#0028
    if(message.content.startsWith(prefix + "emoji")) {   //Xeon#0028
        if(message.author.bot) return;  //Xeon#0028
        var emojiid =  message.content.split(" ").slice(1).join(" ")   //Xeon#0028
        console.log(emojiid) 
        if(emojiid.length < "18" || emojiid.length > "18" || isNaN(emojiid)) return  message.channel.send(`- Usage  //Xeon#0028
${prefix}emoji <EmojiID>`); 
        else    
        message.channel.send("This is the emoji that you requested:-",  //Xeon#0028
          { 
            files: [`https://cdn.discordapp.com/emojis/${emojiid}.png`]  //Xeon#0028
          }) 
        }  
})   
 ...
 client.on("message", message => {  //Xeon#0028
  if (message.channel.type === "dm") {    //Xeon#0028

      message.channel.startTyping();    //Xeon#0028
      setTimeout(() => {    //Xeon#0028
        message.channel.stopTyping();    //Xeon#0028
      }, Math.random() * (1 - 3) + 1 * 1000);  //Xeon#0028
   
  }  
});  

const clans = {};  //Xeon#0028
const system = {};  //Xeon#0028
const level = {};  //Xeon#0028
 
 
client.on('message',async message => {  //Xeon#0028
  if(message.author.bot) return;  //Xeon#0028
  if(message.channel.type === 'dm') return;  //Xeon#0028
 
  let args = message.content.split(' ');  //Xeon#0028
  let random = Math.floor(Math.random() * 5) + 2;  //Xeon#0028
  let author = message.author;
 
  let xpLeft;  //Xeon#0028
  let nameClan;  //Xeon#0028
  let membersClan = [];  //Xeon#0028
  let levelClan = 0;  //Xeon#0028
  if(!system[author.id]) system[author.id] = {clan: 'None',joinedAt: new Date().toLocaleString() ,clanLevel: 0};  //Xeon#0028
 
  if(!level[author.id]) level[author.id] = {level: 1, xp: 1};  //Xeon#0028
 
 
  level[author.id].xp += (+random);  //Xeon#0028
  if(level[author.id].xp >= 300) {  //Xeon#0028
    if(level[author.id].xp > 300) xpLeft = level[author.id].xp - 300;  //Xeon#0028
    level[author.id] = {  //Xeon#0028
      level: level[author.id].level + 1,  //Xeon#0028
      xp: xpLeft  //Xeon#0028
    };
 
  }
  if(message.content.startsWith(prefix + "clan")) {  //Xeon#0028
    if(message.content.split(' ')[0] !== `${prefix}clan`) return;  //Xeon#0028
 
    if(!args[1] || args[1] && args[1] === 'info') {  //Xeon#0028
  let embed = new Discord.RichEmbed()
  .setAuthor('الكلانات', message.author.avatarURL)  //Xeon#0028
  ...
  .setFooter(message.author.username, message.author.avatarURL);  //Xeon#0028
  message.channel.send(embed);
}
 
    if(args[1] && args[1] === 'create') {  //Xeon#0028
      ...
      let awaited = await message.channel.awaitMessages(r => r.author.id === message.author.id, { max: 1, time: 20000, errors: ['time']}).then(collected => {
        ...
        nameClan = collected.first().content;  //Xeon#0028
      });
 
      m = await m.edit('**# جارى عمل الكلان**');  //Xeon#0028
      awaited = await setTimeout(async() => {  //Xeon#0028
        let membersArray = {
          nameClan: {
            array: []
          }
        };
        let members = membersArray.nameClan.array;  //Xeon#0028
        members.push(message.author.id);
        clans[nameClan] = {
          name: nameClan,
          createdAt: new Date().toLocaleString(),  //Xeon#0028
          level: levelClan,
          creator: message.author.id,  //Xeon#0028
          members: members,  //Xeon#0028
          applylist: [],  //Xeon#0028
          admins: []  //Xeon#0028  //Xeon#0028
        };
 
        system[author.id] = {  //Xeon#0028
          clan: nameClan,  //Xeon#0028
          joinedAt: new Date().toLocaleString(),  //Xeon#0028
          clanLevel: 0,  //Xeon#0028
          creator: message.author.id  //Xeon#0028
        };
 
        m = await m.edit('**# تم عمل الكلان بنجاح**');  //Xeon#0028
      }, 2300);
 
    }
    if(args[1] && args[1] === 'invite') {  //Xeon#0028
      if(!system[author.id]) return message.channel.send("**# أنت لست بكلان**");  //Xeon#0028
      let clan = system[author.id].clan;
      if(system[author.id].clan === 'None') return message.channel.send('**# أنت لست بكلان**');  //Xeon#0028
      ...
      let mention = message.mentions.users.first();  //Xeon#0028
      if(!mention) return message.channel.send('**# منشن شخص لدعوته للكلان**');  //Xeon#0028
      if(clans[clan].members.includes(mention.id)) return message.channel.send("**# هذا العضو بالكلان بالفعل**");  //Xeon#0028
      if(clans[clan].members.length === 10) return message.channel.send("**# هذا الكلان وصل للحد الاقصى من الاعضاء يمكنك**");  //Xeon#0028
   //Xeon#0028  //Xeon#0028  //Xeon#0028  //Xeon#0028  //Xeon#0028
      ...
      let awaiting = await message.channel.awaitMessages(r => r.author.id === mention.id, {max: 1, time: 20000, errors:['time']}).then(collected => {
        collected.first().delete().catch();  //Xeon#0028
        if(collected.first().content === 'نعم') {  //Xeon#0028
          clans[clan].members.push(mention.id);  //Xeon#0028
 
          system[author.id].members += 1;  //Xeon#0028
 
 
          system[mention.id] = {  //Xeon#0028
            clan: clan,
            joinedAt: new Date().toLocaleString(),  //Xeon#0028
            clanLevel: 0,
            creator: clans[clan].creator  //Xeon#0028
          };
 
          message.channel.send(`**${message.author} # تم قبول الدعوة**`);  //Xeon#0028
        }  //Xeon#0028
        if(collected.first().content === 'لا') {  //Xeon#0028
          message.channel.send(`**${message.author} # تم رفض الدعوة**`);  //Xeon#0028
        } else if(collected.first().content !== 'نعم' && collected.first().content !== 'لا'){  //Xeon#0028
          return message.channel.send('**# يجب عليك كتابة `نعم` أو `لا`**');  //Xeon#0028
        }  //Xeon#0028
      });  //Xeon#0028
    }  //Xeon#0028
    if(args[1] && args[1] === 'stats') {
      if(system[author.id].clan === 'None') return message.channel.send('**# يجب ان تكون بكلان لأستخدام هذا الأمر**');  //Xeon#0028
      let clan = system[author.id].clan;  //Xeon#0028
      let embed = new Discord.RichEmbed()  //Xeon#0028
        .setAuthor(`${message.author.username} || الكلانات`, message.author.avatarURL)  //Xeon#0028
        .setDescription(`الكلان || \`${clan.toString()}\``)  //Xeon#0028
        embed.addField('» اسم الكلان', clan, true)  //Xeon#0028
        embed.addField('» تاريخ عمل الكلان', clans[clan].createdAt, true);  //Xeon#0028
        embed.addField('» تاريخ دخول الكلان', system[author.id].joinedAt, true)  //Xeon#0028
        embed.addField('» صاحب الكلان', `<@${clans[clan].creator}>`, true);  //Xeon#0028
        embed.addField('» لفل الكلان', clans[clan].level, true);  //Xeon#0028
        embed.addField('» عدد اعضاء الكلان', clans[clan].members.length, true);  //Xeon#0028
        embed.addField('» عدد التقديمات للكلان', clans[clan].applylist.length, true);  //Xeon#0028
        embed.addField('» عدد الادمنية بالكلان', clans[clan].admins.length, true);  //Xeon#0028
        embed.addField('» اعضاء الكلان', `${prefix}clan list || يظهرلك رسالة بها اعضاء الكلان`);  //Xeon#0028
      message.channel.send(embed);  //Xeon#0028
 
    }
    if(args[1] && args[1] === 'join') {  //Xeon#0028  //Xeon#0028
    let clanName = message.content.split(' ').slice(2).join(" ");  //Xeon#0028
    if(system[author.id].clan !== 'None') return message.channel.send("**# يجب أن لا تكون بكلان**");  //Xeon#0028
    if(!args[2]) return message.channel.send("**# يجب عليك كتابة اسم الكلان**");  //Xeon#0028
    if(!clans[clanName]) return message.channel.send("**# هذا الكلان غير موجود**");  //Xeon#0028
    if(clans[clanName].applylist.includes(message.author.id)) return message.channel.send("**# لقد قدمت على دخول هذا الكلان مسبقا");  //Xeon#0028
   //Xeon#0028  //Xeon#0028  //Xeon#0028
    clans[clanName].applylist.push(message.author.id);  //Xeon#0028
    message.channel.send("**# لقد تم التقديم على دخول الكلان , سيتم الرد عليك من قبل احد ادارة الكلان**");  //Xeon#0028
 
  }
    if(args[1] && args[1] === 'accept') {  //Xeon#0028
      let mention = message.mentions.users.first();
      if(system[author.id].clan === 'None') return message.channel.send("**# يجب عليك ان تكون بكلان لأستخدام هذا الأمر**");  //Xeon#0028
      ...
      if(!system[mention.id]) system[mention.id] = {clan: 'None',joinedAt: new Date().toLocaleString() ,clanLevel: 0};  //Xeon#0028
      ...
      clans[system[author.id].clan].applylist.shift(mention.id);  //Xeon#0028
      clans[system[author.id].clan].members.push(mention.id);  //Xeon#0028
      let clan = system[author.id].clan;  //Xeon#0028
   ...
   //Xeon#0028
      ...
      mention.send(`**# \`${system[author.id].clan}\`, لقد تم قبولك بالكلان**`).catch();  //Xeon#0028
      message.channel.send(`**# \`${mention.username}\`, لقد تم قبول الشخص ودخوله للكلان**`);  //Xeon#0028
    }  //Xeon#0028
    if(args[1] && args[1] === 'decline') {  //Xeon#0028
      let mention = message.mentions.users.first();  //Xeon#0028
      ...
      system[mention.id] = {  //Xeon#0028
        clan: clans[system[author.id].clan],  //Xeon#0028
        joinedAt: new Date().toLocaleString(),
        clanLevel: 0  //Xeon#0028
      };
 
 
      mention.send(`**# \`${system[author.id].clan}\`, لقد تم رفض دخولك للكلان**`).catch();  //Xeon#0028
      message.channel.send(`**# \`${mention.username}\`, لقد تم رفض دخول الشخص للكلان**`);  //Xeon#0028
   //Xeon#0028
    }
    if(args[1] && args[1] === 'promote') {
      ...
      if(system[mention.id].clan === 'None') return message.channel.send("**# هذا الشخص ليس بكلان**");  //Xeon#0028
      if(!clans[system[author.id].clan].members.includes(mention.id)) return message.channel.send("**# هذا الشخص ليس بالكلان**");  //Xeon#0028
      if(clans[system[author.id].clan].admins.includes(mention.id)) return message.channel.send("**# هذا العضو لديه ادمن بالفعل**");  //Xeon#0028
      if(mention.id === message.author.id) return message.channel.send("**# لا يمكنك اعطاء نفسك ترقية**");  //Xeon#0028
 
      clans[system[author.id].clan].admins.push(mention.id);  //Xeon#0028
 
 
      mention.send(`**# \`${system[author.id].clan}\`, لقد تم ترقيتك الى ادمن**`).catch();  //Xeon#0028
      message.channel.send(`**# \`${mention.username}\`, لقد تم ترقية العضو الى رتبة ادمن**`);  //Xeon#0028
    }  //Xeon#0028
    if(args[1] && args[1] === 'demote') {  //Xeon#0028
      let mention = message.mentions.users.first();  //Xeon#0028
      if(system[author.id].clan === 'None') return message.channel.send("**# يجب ان تكون بكلان لأستخدام هذا الأمر**");  //Xeon#0028
      if(clans[system[author.id].clan].creator !== message.author.id) return message.channel.send("**# هذا الأمر لضاحب الكلان فقط**");  //Xeon#0028
      if(!mention) return message.channel.send("**# يجب عليك منشنة عضو بالكلان لأعطائه الترقية**");  //Xeon#0028
      if(!system[mention.id]) system[mention.id] = {clan: 'None',joinedAt: new Date().toLocaleString() ,clanLevel: 0};  //Xeon#0028
   //Xeon#0028
      if(system[mention.id].clan === 'None') return message.channel.send("**# هذا الشخص ليس بكلان**");  //Xeon#0028
      if(!clans[system[author.id].clan].members.includes(mention.id)) return message.channel.send("**# هذا الشخص ليس بالكلان**");  //Xeon#0028
      if(!clans[system[author.id].clan].admins.includes(mention.id)) return message.channel.send("**# هذا الشخص ليس ادمن بالكلان**");  //Xeon#0028
      if(mention.id === message.author.id) return message.channel.send("**# لا يمكنك اعطاء نفسك ترقية**");  //Xeon#0028
   //Xeon#0028
      clans[system[author.id].clan].admins.shift(mention.id);  //Xeon#0028
...
reaction1.on("collect", r => {
  const embed = new Discord.RichEmbed()
      ...
      .setColor("#000000")
      .setDescription(`
      :busts_in_silhouette:***__Public CMD__***:loudspeaker: 
**
『-ping ====> لمعرفة سرعة اتصال البوت
『-perms ====> show your perms
『-topinv ====> show top invites
『-report ====> report someone
『-Minv ====> show your invites number
『-color ====> make a color
『-emojilist ====> server emojis list
『-gRole Role name ====> show you role infos
『-bot ====> show you bot infos
『-say ====> repeat your message
『-savatar ====> server avatar
『-id ====> account infos
『-avatar ====> profile picture
『-embed ====> create an embed
『-rules ====> show you server rules
『-v2min ====> create a temp vc channel
『-inv ====> invite the bot to your server
『 By ====> Xeon#0028
**
`)
...
reaction2.on("collect", r => {
  ...
  .setThumbnail('https://cdn.discordapp.com/attachments/553862087382925313/556036868492230667/logo-admin-png-4.png')
      .setColor("#000000")
      .setDescription(`
      :key:***__Administration CMD__***:crown: 
**
『-bc ====> Broadcast To All Server Members
『-rolebc ====> Broadcast To A Specified Role
『-setbot ====> Create A VC channel With Server Bots Number
『-bans ====> Show The Banned Members List
『-setmember ====> Create A VC channel With Server Members Number
『-leave ====> Kick The Bot From the server
『-ranks ====> Show You server roles
『-vonline ====> create an online VC channel
『-schannel ====> Show all the channels in the server
『-sr (your choice) ====> to change server's name
『-kv ====> kick a member from vc channel
『-hchannel ====> hide the channel
『-talk ====> talk like a bot
『-count ====> count server members
『-setlog ====> create a log channel
『-server ====> show you infos about the server
『-movall ====> move all the members to your vc channel
『-sug ====> make some suggestions
『-rooms ====> server channels list
『-dc ====> delete all the channels
『-dr ====>  above all the ranks
『-allbots ====> show all server bots
『-move ====> move a member to a specified channel
『-giveaway ====> create a giveaway
『-role @mention rolename ====> give a role to a specified member
『-role all rolename ====> give a role to all members
『-role humans rolename ====> give a role to members
『-role bots rolename ====> give a role to bots
『-members ====> server members status
『-send ====> send a message to the mentioned member
『-clr <numbr> ====> clear messages with a specified number
...
『-mutechannel ====> mute the channel
『-unmutechannel ====> unmute the channel
